...
// Generated 21-Apr-2021 13:25:41
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"SMC_original_ROS2020b\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(SMC_original_ROS2020b_M, (NULL));
        SMC_original_ROS2020b_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(SMC_original_ROS2020b_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(SMC_original_ROS2020b_M)
                == (NULL)) || rtmGetStopRequested(SMC_original_ROS2020b_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        SMC_original_ROS2020b_terminate();
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(SMC_original_ROS2020b_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
